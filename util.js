		...
		if (r){
			if (r[2]){
				...
				if (dir_names[name] == null){
					dir_names[name] = i;
					ro.directives.push({name:name, value:value});
				}
			}
		}
		else {
...
function csp_directives2str(directives)
{
	let a = [];
	for (let i = 0 ; i < directives.length ; i++){
		a.push(directives[i].name + directives[i].value);
	}
	return a.join(";");
}

function cspMerge(csp_str, directives)
{
	let dir_names = {}, a = csp_str.split(";");
	for (let i = 0 ; i < a.length ; i++){
		let r = a[i].match(CSP_DIRECTIVE_REX);
	...
	for (let i = 0 ; i < directives.length ; i++){
		if (dir_names[directives[i].name] != null)
			a[dir_names[directives[i].name]] += directives[i].value;
		else
			a.push(directives[i].name + directives[i].value);
	}
	return a.join(";");
